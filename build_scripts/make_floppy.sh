...
STAGE2_SIZE=$(stat -c%s ${BUILD_DIR}/sec_stg.bin)
echo ${STAGE2_SIZE}
...
# install bootloader
dd if=${BUILD_DIR}/fst_stg.bin of=$TARGET conv=notrunc bs=1 count=3 2>&1 >/dev/null
dd if=${BUILD_DIR}/fst_stg.bin of=$TARGET conv=notrunc bs=1 seek=90 skip=90 2>&1 >/dev/null
dd if=${BUILD_DIR}/sec_stg.bin of=$TARGET conv=notrunc bs=512 seek=1 #>/dev/null

# write lba address of sec_stg to bootloader
