...
x86_diskWrite:
    ...
    mov cx, [ebp + 10]          ; cx - cylinder and sector
    mov dh, [ebp + 12]          ; dh - head
    mov al, [ebp + 14]          ; al - count

    linear_to_seg_offset [ebp + 16], es, ebx, bx  ; Pointer to dataIn

    ; call int13h for write operation
    mov ah, 03h   ; Set AH register to 3 for write
    stc
    int 13h
...
global x86_video_getVbeModeInfo
x86_video_getVbeModeInfo:
    ...
    mov ax, 0x4f01
    mov ecx, [bp + 8]
    linear_to_seg_offset [bp + 12], es, edi, di
    int 10h
...
global x86_video_setVbeMode
x86_video_setVbeMode:
