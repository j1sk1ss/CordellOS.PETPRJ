...
int FATRead(unsigned int clusterNum) {
	if (clusterNum < 2 || clusterNum >= total_clusters) {
		printf("Function FATRead: invalid cluster number!\n");
		return -1;
	}
	...
	if (fat_type == 32) {
		...
		if (cluster_data == NULL) {
			printf("Function FATRead: Could not read sector that contains FAT32 table entry needed.\n");
			return -1;
		}
        ...
        char* cluster_data = ATA_read_sector(fat_sector, 1);
		if (cluster_data == NULL) {
			printf("Function FATRead: Could not read sector that contains FAT16 table entry needed.\n");
			return -1;
		}
		...
		printf("Function FATRead: Invalid fat_type value. The value was: %i\n", fat_type);
		return -1;
	}
}
...
int FATWrite(unsigned int clusterNum, unsigned int clusterVal) {
	...
	if (clusterNum < 2 || clusterNum >= total_clusters) {
		printf("Function FATWrite: invalid cluster number!\n");
		return -1;
	}
	...
	if (fat_type == 32) {
		...
		if (ATA_read_sector(fat_sector, 1) == NULL) {
			printf("Function FATWrite: Could not read sector that contains FAT32 table entry needed.\n");
			return -1;
		}
		...
		*(unsigned int*)&FAT_table[ent_offset] = clusterVal;
		...
		if (ATA_write_sector(fat_sector, 1, (char*)(DISK_WRITE_LOCATION + bytes_per_sector)) != 1) {
			printf("Function FATWrite: Could not write new FAT32 cluster number to sector.\n");
			return -1;
		}
        ...
        char* sector_data = ATA_read_sector(fat_sector, 1);
		if (sector_data == NULL) {
			printf("Function FATWrite: Could not read sector that contains FAT16 table entry needed.\n");
			return -1;
		}
		...
		*(unsigned short*)&FAT_table[ent_offset] = (unsigned short)clusterVal;
		...
		memcpy((char*)DISK_WRITE_LOCATION, &FAT_table, bytes_per_sector);
		if (ATA_write_sector(fat_sector, 1, (char*)(DISK_WRITE_LOCATION + bytes_per_sector)) != 1) {
			printf("Function FATWrite: Could not write new FAT16 cluster number to sector.\n");
			return -1;
		}
		...
		printf("Function FATWrite: Invalid fat_type value. The value was: %i\n", fat_type);
		return -1;
	}
}
...
unsigned int allocateFreeFAT() {
	...
	if (fat_type == 32) {
		...
		printf("Function allocateFreeFAT: fat_type is not valid!\n");
	...
	while (cluster < total_clusters) {
		...
		if (clusterStatus == free_cluster) {
			...
			if (FATWrite(cluster, end_cluster) == 0)
				...
				printf("Function allocateFreeFAT: Error occurred with FATWrite, aborting operations...\n");
				return bad_cluster;
			}
		}
		else if (clusterStatus < 0) {
			printf("Function allocateFreeFAT: Error occurred with FATRead, aborting operations...\n");
			return bad_cluster;
		}
...
char* clusterRead(unsigned int clusterNum, unsigned int clusterOffset) {
	if (clusterNum < 2 || clusterNum >= total_clusters) {
		printf("Function clusterRead: Invalid cluster number!\n");
		return NULL;
	}
    ...
    if (cluster_data == NULL) {
        printf("Function clusterRead: An error occurred with ATA_read_sector\n");
        return NULL;
    }
...
int clusterWrite(void* contentsToWrite, unsigned int contentSize, unsigned int contentBuffOffset, unsigned int clusterNum) {
	if (clusterNum < 2 || clusterNum >= total_clusters) {
		printf("Function clusterWrite: Invalid cluster number!\n");
		return -1;
	}
    ...
    if (ATA_write_sector(start_sect, contentSize, contentsToWrite) == -1) {
        printf("Function clusterWrite: An error occurred with ATA_write_sector, the area in sector ");
        return -1;
    }
	else return 0;
}
...
int directoryList(const unsigned int cluster, unsigned char attributesToAdd, BOOL exclusive) {
	if (cluster < 2 || cluster >= total_clusters) {
		printf("Function directoryList: Invalid cluster number!\n");
		return -1;
	}
    ...
    char* cluster_data = clusterRead(cluster, 0);
	if (cluster_data == NULL) {
		printf("Function directoryList: clusterRead encountered an error. Aborting...\n");
		return -1;
	}
	...
	while (1) {
		if (file_metadata->file_name[0] == ENTRY_END) {
			...
			else //search next cluster in directory
			{
				...
				{
					printf("Function directoryList: FATRead encountered an error. Aborting...\n");
					return -1;
				}
				else
...
int directorySearch(const char* filepart, const unsigned int cluster, directory_entry_t* file, unsigned int* entryOffset) {
	if (cluster < 2 || cluster >= total_clusters) {
		printf("Function directorySearch: Invalid cluster number!\n");
		return -1;
	}
    ...
    char* cluster_data = clusterRead(cluster, 0);
	if (cluster_data == NULL) {
		printf("Function directorySearch: clusterRead encountered an error. Aborting...\n");
		return -1;
	}
	...
	while (1)
	{
		...
		{
			if (meta_pointer_iterator_count < bytes_per_sector * sectors_per_cluster / sizeof(directory_entry_t) - 1) //if the pointer hasn't iterated outside of what that cluster can hold (the 1 is to prevent strncmp from the line above from reading past the cluster boundary)
			{
				file_metadata++;
				meta_pointer_iterator_count++;
			}
			else
			{
				...
				{
					printf("Function directorySearch: FATRead encountered an error. Aborting...\n");
					return -1;
				}
				else
...
int directoryAdd(const unsigned int cluster, directory_entry_t* file_to_add) {
	if (testIfFATFormat(file_to_add->file_name) != 0) {
		printf("Function directoryAdd: file name supplied is invalid!");
		return -1;
	}
    ...
    char* cluster_data = clusterRead(cluster, 0);
	if (cluster_data == NULL) {
		printf("Function directoryAdd: clusterRead encountered an error. Aborting...\n");
		return -1;
	}
	...
	while (1) {
		if (file_metadata->file_name[0] != ENTRY_FREE && file_metadata->file_name[0] != ENTRY_END) //if the file directory slot isn't free
		{
			...
			else
			{
				...
				if ((next_cluster >= END_CLUSTER_32 && fat_type == 32) || (next_cluster >= END_CLUSTER_16 && fat_type == 16) || (next_cluster >= END_CLUSTER_12 && fat_type == 12)) //no free spaces left in the directory cluster, and no more clusters to search. Allocate a new one.
				{
					...
					{
						printf("Function directoryAdd: allocation of new cluster failed. Aborting...\n");
						return -1;
					}
					...
					{
						printf("Function directoryAdd: extension of the cluster chain with new cluster failed. Aborting...\n");
						return -1;
					}
				}
		...
		else
		{
			...
			for (dot_checker = 0; dot_checker < 11; dot_checker++)
			{
				...
				{
					printf("Function directoryAdd: Invalid file name!");
					return -1;
				}
			}
			...
			printf("Function directoryAdd: the new cluster for the file is ");
			...
			if (clusterWrite((void *)DISK_WRITE_LOCATION, bytes_per_sector * sectors_per_cluster, 0, cluster) != 0)
			{
				printf("Function directoryAdd: Writing new directory entry failed. Aborting...\n");
				return -1;
			}
			return 0;
		}
...
int getFile(const char* filePath, char** fileContents, directory_entry_t* fileMeta, unsigned int readInOffset) {
	if (fileContents == NULL || fileMeta == NULL) {
		printf("Function getFile: an argument was invalid!\n");
		return -1;
	}
	...
	if (fat_type == 32)
		active_cluster = ext_root_cluster; //holds the cluster to be searched for directory entries related to the path
	else {
		printf("Function getFile: FAT16 and FAT12 are not supported!\n");
		return -1;
	}
	...
	for (iterator = 3; filePath[iterator - 1] != '\0'; iterator++) {
		if (filePath[iterator] == '\\' || filePath[iterator] == '\0') {
			...
			if (retVal == -2) //no directory matching found
				...
				printf("Function getFile: An error occurred in directorySearch. Aborting...\n");
				return retVal;
			}
	...
	*fileMeta = file_info; //copy fileinfo over
	...
	if ((file_info.attributes & FILE_DIRECTORY) != FILE_DIRECTORY) {
		if (readInOffset < 1 || (readInOffset * (unsigned short)bytes_per_sector * (unsigned short)sectors_per_cluster) + file_info.file_size > 262144) //prevent offsets that extend into FATRead's working range or outside the allocated BIOS int13h space
		...
		while (cluster < END_CLUSTER_32) {
			char* content_part = clusterRead(cluster, clusterReadCount + readInOffset);
			...
			if (cluster == BAD_CLUSTER_32) {
				printf("Function getFile: the cluster chain is corrupted with a bad cluster. Aborting...\n");
				return -1;
			}
			else if (cluster == -1 ) {
				printf("Function getFile: an error occurred in FATRead. Aborting...\n");
				return -1;
			}
		}
...
int putFile(const char* filePath, char** fileContents, directory_entry_t* fileMeta) {
	...
	if (fat_type == 32)
		...
		printf("Function putFile: FAT16 and FAT12 are not supported!\n");
		return -1;
	}
	...
	if (strcmp(filePath, "C:\\") == 0) {
		...
		for (iterator = 3; filePath[iterator - 1] != '\0'; iterator++) {
			if (filePath[iterator] == '\\' || filePath[iterator] == '\0') {
				...
				if (retVal == -2) {
					printf("Function putFile: No matching directory found. Aborting...\n");
					return -2;
				}
				else if (retVal == -1) {
					printf("Function putFile: An error occurred in directorySearch. Aborting...\n");
					return retVal;
				}
	...
	if ((file_info.attributes & FILE_DIRECTORY) == FILE_DIRECTORY) {
		if (directoryAdd(active_cluster, fileMeta) != 0) {
			printf("Function putFile: directoryAdd encountered an error. Aborting...\n");
			return -1;
		}
		...
		if (retVal == -2) {
			printf("Function putFile: directoryAdd did not properly write the new file's entry to disk. Aborting...\n");
			return -2;
		}
		else if (retVal != 0) {
			printf("Function putFile: directorySearch has encountered an error. Aborting...\n");
			return -1;
		}

		active_cluster = GET_CLUSTER_FROM_ENTRY(file_info);
		
		printf("Cluster of Entry: %i\n", active_cluster);
		...
		while (dataLeftToWrite > 0) {
			...
			if (dataLeftToWrite >= bytes_per_sector * sectors_per_cluster)
				dataWrite = bytes_per_sector * sectors_per_cluster + 1;
			else
			...
			if (clusterWrite(*fileContents + (fileMeta->file_size - dataLeftToWrite), dataWrite, 1, active_cluster) != 0) {
				printf("Function putFile: clusterWrite encountered an error. Aborting...\n");
				return -1;
			}
			...
			if (dataLeftToWrite == 0)
				...
				printf("Function putFile: An undefined value has been detected. Aborting...\n");
				return -1;
			}
			...
			if ((new_cluster == BAD_CLUSTER_32 && fat_type == 32) || (new_cluster == BAD_CLUSTER_16 && fat_type == 16) || (new_cluster == BAD_CLUSTER_12 && fat_type == 12)) {
				printf("Function putFile: allocateFreeFAT encountered an error. Aborting...\n");
				return -1;
			}
			if (FATWrite(active_cluster, new_cluster) != 0) {
				printf("Function putFile: FATWrite encountered an error. Aborting...\n");
				return -1;
			}
		...
		//printf ("Function putFile: Success!\n");
		return 0; //file successfully written
	}
	else {
		printf ("Function putFile: Invalid path!\n");
